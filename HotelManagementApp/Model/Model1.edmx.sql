...
-- --------------------------------------------------
-- Date Created: 11/29/2022 22:08:28
-- Generated from EDMX file: D:\University\LTTQ\modified\HotelManagementApp\HotelManagementApp\Model\Model1.edmx
-- --------------------------------------------------
...
GO
IF OBJECT_ID(N'[dbo].[FK_RoomsReservation_BillDetail_BillDetailID]', 'F') IS NOT NULL
    ALTER TABLE [dbo].[RoomsReservations] DROP CONSTRAINT [FK_RoomsReservation_BillDetail_BillDetailID];
GO
IF OBJECT_ID(N'[dbo].[FK_FL_RoomsReservations]', 'F') IS NOT NULL
    ALTER TABLE [dbo].[Rooms] DROP CONSTRAINT [FK_FL_RoomsReservations];
GO
...
ALTER TABLE [dbo].[RoomsReservations]
ADD CONSTRAINT [FK_RoomsReservation_BillDetail_BillDetailID]
    FOREIGN KEY ([IDBillDetail])
    REFERENCES [dbo].[BillDetails]
        ([ID])
    ON DELETE CASCADE ON UPDATE NO ACTION;
...
-- Creating non-clustered index for FOREIGN KEY 'FK_RoomsReservation_BillDetail_BillDetailID'
CREATE INDEX [IX_FK_RoomsReservation_BillDetail_BillDetailID]
...
-- Creating foreign key on [IDRoomsReservation] in table 'Rooms'
ALTER TABLE [dbo].[Rooms]
ADD CONSTRAINT [FK_FL_RoomsReservations]
    FOREIGN KEY ([IDRoomsReservation])
    REFERENCES [dbo].[RoomsReservations]
        ([ID])
    ON DELETE NO ACTION ON UPDATE NO ACTION;
GO

-- Creating non-clustered index for FOREIGN KEY 'FK_FL_RoomsReservations'
CREATE INDEX [IX_FK_FL_RoomsReservations]
ON [dbo].[Rooms]
    ([IDRoomsReservation]);
GO
